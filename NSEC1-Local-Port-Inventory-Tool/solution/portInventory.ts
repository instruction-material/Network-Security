...
export function assessListenerExposure(
    ...
    if (listener.bindScope === "public" && !listener.expectsAuthentication) {
        return {
            risk: "high",
            reason: "Public unauthenticated listeners should be justified and tightly controlled.",
        };
    }
    ...
    if (listener.bindScope === "public" && isManagementPort(listener.port)) {
        return {
            risk: "high",
            reason: "Public management ports need explicit review, rate limits, and log visibility.",
        };
    }
    ...
    if (listener.bindScope === "lan") {
        return {
            risk: "medium",
            reason: "LAN-visible services still widen the attack surface and should remain intentional.",
        };
    }

    return {
        risk: "medium",
        reason: "Unknown or partially authenticated listeners deserve manual verification.",
    };
}
